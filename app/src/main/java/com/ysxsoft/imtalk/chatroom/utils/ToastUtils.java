...
public class ToastUtils {
    ...
    public static void showErrorToast(ErrorCode errorCode) {
        ...
        String message = MyApplication.mcontext.getResources().getString(errorMsgResourceId);
        if (lastToast != null) {
            lastToast.setText(message);
        } else {
            lastToast = Toast.makeText(MyApplication.mcontext, message, Toast.LENGTH_SHORT);
        }
        lastToast.show();
    }
    ...
    public static void showToast(int resourceId) {
        showToast(MyApplication.mcontext.getResources().getString(resourceId));
    }
